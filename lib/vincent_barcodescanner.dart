...
class _BarcodeScannerScreen2State extends State<BarcodeScannerScreen2>
  ...
  // Fixed scan area dimensions
  static const double scanAreaWidth = 240;
  static const double scanAreaHeight = 75;
  ...
  void _processScannedCode(BarcodeCapture capture, Rect scanWindow) {
    ...
    double weightedConfidence = math.min(1.0, 0.1 + (_scanCount / 1.5) * 0.7);

    setState(() {
      _confidenceLevel = weightedConfidence;
      _validBarcodeDetected = _scanCount >= 6 && _confidenceLevel > 0.7;
    });

    // Reset scan count if no scan in 1s
    _scanResetTimer?.cancel();
    _scanResetTimer = Timer(const Duration(milliseconds: 400), () {
      setState(() {
        _scanCount = 0;
    ...
    if (_confidenceLevel > 0.7 && _scanCount >= 6) {
      _scanResetTimer?.cancel();
      _foundBarcode(code);
    }
  }
  ...
  String get _positionBarcodeText =>
      _currentLanguageFlag == 2
          ? "バーコードをフレーム内に配置してスキャンしてください"
          : "Position the barcode within the frame to scan";

  String get _flashOnTooltip =>
      _currentLanguageFlag == 2 ? "フラッシュオン" : "Flash on";

  String get _flashOffTooltip =>
      _currentLanguageFlag == 2 ? "フラッシュオフ" : "Flash off";

  String get _frontCameraTooltip =>
      _currentLanguageFlag == 2 ? "フロントカメラ" : "Front camera";

  String get _rearCameraTooltip =>
      _currentLanguageFlag == 2 ? "リアカメラ" : "Rear camera";

  String get _accuracyText => _currentLanguageFlag == 2 ? "精度" : "Accuracy";
  ...
  Widget build(BuildContext context) {
    ...
    // Adjust scanWindow dimensions based on orientation
    final scanWindow = Rect.fromCenter(
      center: Offset(screenSize.width / 2, screenSize.height / 2),
      width: isPortrait ? scanAreaWidth : scanAreaHeight,
      height: isPortrait ? scanAreaHeight : scanAreaWidth,
    );
        ...
        body: Center(
          child: Column(
            ...
            children: [
              ...
              Text(
                "Go to Permissions > Select CAMERA > Select Allow on while using the app",
        ...
        children: [
          GestureDetector(
            ...
            child: MobileScanner(
              ...
              onDetect: (capture) {
                _processScannedCode(capture, scanWindow);
              },
            ),
          ),
